// Kanban Board Template Function
...
function createColumnTemplate(column: ColumnConfig): string {
  const dataColumn =
    column.status === "inprogress" ? "in-progress" : column.status;
...
function createClearDataButton(): string {
...
export function createBoardTemplate(): string {
  return `
